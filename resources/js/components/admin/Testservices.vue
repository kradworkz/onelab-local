                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-centered table-nowrap">
                                    ...
                                    <tbody>
                                        <tr v-for="test in testservices" v-bind:key="test.id">
...
<script>
    ...
    export default {
        ...
        methods : {
            ...
            fetchList(page_url) {
                axios.get('/request/admin/testservice/list')
                .then(response => {
                    this.testservicelists = response.data.data;
                })
                .catch(err => console.log(err));
            },

            syncTestservice(page_url) 
            {
                axios.get('https://one.main/api/sync/testservices')
                .then(response => {
                    this.testserviceSync = response.data.data;
                    
                    for(var i=0; i < this.testserviceSync.length; i++){
                        if(this.testservicelists.length > 0){
                           
                            let x = this.testservicelists.some(
                                testservice => (
                                    testservice.id === this.testserviceSync[i].id
                                )
                            )
                            if(!x){
                                this.syncable.push(this.testserviceSync[i]);
                            }
                        }else{
                            this.syncable.push(this.testserviceSync[i]);
                        }
            ...
            search() {
                let vm = this;
                axios.post(this.currentUrl + '/request/admin/testservices/search', {
